...
- When creating the action group, call it `query-athena`. Select Lambda function `bedrock-agent-txtsql-action`. Next, select the schema file `athena-schema.json` from the S3 bucket `athena-destination-store-{alias}`. 
